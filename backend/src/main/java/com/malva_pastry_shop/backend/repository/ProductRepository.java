...
public interface ProductRepository extends JpaRepository<Product, Long> {
    ...
    // Lo mismo para todas las categorias de una pagina de una sola vez, para
    // que el listado no emita una consulta por fila. Las categorias sin
    // productos NO salen en el resultado: la ausencia es cero.
    @Query("""
            select p.category.id as id, count(p) as total
            from Product p
            where p.category.id in :categoryIds and p.deletedAt is null
            group by p.category.id
            """)
    List<IdCount> countActiveByCategoryIds(@Param("categoryIds") Collection<Long> categoryIds);
